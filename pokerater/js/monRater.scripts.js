...
function drawImage() {
  ...
  document.getElementById("pokemon_name").innerHTML = pokemon[pk_id].NameJp;
  ...
  var path = document.createElementNS("http://www.w3.org/2000/svg", "text");
  path.innerHTML = "レベル " + pokemon_level;
  path.setAttribute("font-size", "12px");
  path.setAttribute("fill", "hsla(0, 0%, 0%, .33)");
  path.setAttribute("x", cr_min_x);
  ...
  var path = document.createElementNS("http://www.w3.org/2000/svg", "text");
  path.innerHTML = "平均" + (per_diff_br > 0?"+":"") + per_diff_br.toPrecision(3) + "%";
  path.setAttribute("font-size", "12px");
  path.setAttribute("fill", "hsla(0, 0%, 0%, .66)");
  path.setAttribute("x", (cr_max_x - cr_min_x) * .25 + cr_min_x);
  if (card_width < 450) {
  ...
  var path = document.createElementNS("http://www.w3.org/2000/svg", "text");
  path.innerHTML = "平均" + (per_diff_cp > 0?"+":"") + per_diff_cp.toPrecision(3) + "%";
  path.setAttribute("font-size", "12px");
  path.setAttribute("fill", "hsla(0, 0%, 0%, .66)");
  path.setAttribute("x", (cr_max_x - cr_min_x) * .5 + cr_min_x);
  path.setAttribute("y", cr_center_y + 94);
  ...
  var path = document.createElementNS("http://www.w3.org/2000/svg", "text");
  path.innerHTML = "平均" + (per_diff_sta > 0?"+":"") + per_diff_sta.toPrecision(3) + "%";
  path.setAttribute("font-size", "12px");
  path.setAttribute("fill", "hsla(0, 0%, 0%, .66)");
  path.setAttribute("x", (cr_max_x - cr_min_x) * .75 + cr_min_x);
  if (card_width < 450) {
...
function updateIVNumbers(rating_br, rating_sta) {
    ...
    switch (true) {
        case (Math.round((rating_br/100)*30) > 30 || Math.round((rating_sta/100)*15) > 15):
            textRating = "あばばばば。測定ミス！<br>STEP2のスライダー位置をもうちょっと上げたほうが良いかも。";
            break;
        case (rating_br + rating_sta < 50):
            textRating = "...逆に珍しいかも？<br>思い出のポケモンだったら<br>大事にしてあげてね。<br>評価：最弱";
            break;
        case (rating_br + rating_sta < 100):
            textRating = "平均よりちょっと低め。<br>ウィロー博士に送ったほうがいいかも...。<br>評価：微妙";
            break;
        case (rating_br + rating_sta < 150):
            textRating = "なかなかのポケモンだよ！<br>大事にすれば頑張ってくれるかも！<br>評価：普通";
            break;
        case (rating_br + rating_sta < 199):
            textRating = "かなり強いポケモンだよ！！<br>ジムバトルで活躍してくれるはず！<br>評価：優良";
            break;
        case (rating_br + rating_sta >= 200):
            textRating = "文句なし！最高レベルの個体値！<br>本当にでたの！？<br>評価：最強";
            break;
        default:
            textRating = "うーん。なにか設定がおかしかったようです。<br>もう一度HPとCP、\"ほしのすな\"と\"アメ\"が正しいか確認してみてください。";
            break;
    }
